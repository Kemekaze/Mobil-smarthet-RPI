...
public class PhidgetsSensor{
	...
	private String tableSound = "sound", tableMotion = "motion";
		...
		motionSensor.addSpatialDataListener(new SpatialDataListener() {
			public void data(SpatialDataEvent sde) {
				...
				for(j=0;j<sde.getData().length;j++)
				{
					...
					{
						/*
						DB.get().addSensorvalue(tableMotion, new double[]{ 
			...
			public void sensorChanged(SensorChangeEvent sensorChangeEvent){
				try
				{
					...
					if((lastdB-tempdB) > soundSensitivity){
						/*
						 * DB.get().addSensorvalue(tableSound, lastdB);
						 * DB.get().addSensorvalue(tableSound, tempdB);
						 */
	...
	/**
	 * sets the name of the table which the motionsensor writes to in the DB.
	 * @param name table name 
	 */
	public void setMotionTableName(String name){
		tableMotion = name;
	}
	 ...
	 * @param name table name
	 */
	public void setSoundTableName(String name){
		tableSound = name;
	}
